...
const oddArray = randomNumbers.filter(numbers => numbers % 2 !== 0)
...
const numbersLess501 = crazyNumbers.reduce((accumulator, item) =>
{
  if (item < 501)
  {
    accumulator += 1;
...
const gameList = cart.reduce((acc, item) => acc += `- ${item.name}\n`, '');
// const gameList = cart.reduce((acc, item) => `${acc}- ${item.name}\n`, ''); //! também está correto
...
const tvShowNames = tvShows.map(tvShows => tvShows.name);
