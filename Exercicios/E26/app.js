...
{
  let day = (date.getDay() < 10) ? `0${date.getDate()}` : date.getDay();
...
const formatDateInfo = date =>
{
  ...
  let dayOfWeek = ['domingo', 'segunda', 'terça', 'quarta', 'quinta', 'sexta', 'sábado'];
  let month = ['janeiro', 'fevereiro', 'março', 'abril', 'maio', 'junho', 'julho', 'agosto', 'setembro', 'outubro', 'novembro', 'dezembro'];
  return `${formatTime} - ${dayOfWeek[date.getDay()]}, ${date.getDate()} de ${month[date.getMonth()]} de ${date.getFullYear()}`;
};
...
const updateClock = () =>
{
  ...
  {
    const templateTime = (String(time).length === 1) ? `0${time}` : time;
    return `${templateTime}`
  }
